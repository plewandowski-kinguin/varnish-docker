...
if (req.url ~ "^.*apimobile/v[1-9]/(home|blog/post/|blog/list|categories/attribute/.*/(page|type)|category/.*/(info|products)|category/search|catalogsearch|product/.*/info|locales|payment_currencies).*$") {
return(lookup);
}
...
if (req.url ~ "[?&](gclid|cx|ie|cof|siteurl)=") {
set req.url = regsuball(req.url, "(?:(\?)?|&)(?:gclid|cx|ie|cof|siteurl)=[^&]+", "\1");
...
if (req.http.Host && req.http.Host ~ "^www\..+") {
...
unset resp.http.X-Varnish-Store;
